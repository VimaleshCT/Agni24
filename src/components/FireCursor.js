  ...
  useEffect(() => {
    ...
    const setInitialStyles = () => {
      circlesRef.current.forEach((circle, index) => {
        if (circle) { // Ensure circle is defined
          circle.x = 0;
          circle.y = 0;
    ...
    const animateCircles = () => {
      if (circlesRef.current.every(circle => circle)) { // Check that all circles are populated
        let x = coords.current.x;
        let y = coords.current.y;

        circlesRef.current.forEach((circle, index) => {
          if (circle) { // Ensure circle is defined before accessing
            ...
            if (nextCircle) { // Ensure nextCircle is defined
